...
import { downloadTourLeadCSV } from '@/lib/exports';
import { usePermissions } from '@/hooks/use-permissions';

function TourLeadExportPage() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      setSuccess('Tour lead data exported successfully');
    } catch (err) {
      setError('Failed to export tour lead data');
      console.error(err);
    } finally {
        ...
        <Stack spacing={3}>
          <Box>
            <Typography variant="h4" sx={{ mb: 1 }}>
              Export Tour Lead Data
            </Typography>
            <Typography variant="body2" color="text.secondary">
              Download tour lead information as CSV
            </Typography>
          </Box>
...
export default TourLeadExportPage;
